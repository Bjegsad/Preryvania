...
section .code: ;сам код
	...
	new09h:     		;------------; новый обработчик прерывания INT-09h ----------------------------|
		...
		popa 				; Извлечение регистров общего назначения из стека
	...
	.down: ; стрелка вниз
		popa ;Извлечение регистров общего назначения из стека
		call near .sys ;для вызова подпрограммы, находящейся в другом сегменте памяти.
		mov dl, 25 ;передача значения 25 в регистр DL
		mov ah, 02h ;;передача значение 02h в регистр ah
		int 21h ;прерывание 
		iret ;используется для возврата из прерывания.
	...
	.right: ; стрелка вправо
		popa ;Извлечение регистров общего назначения из стека
		call near .sys ;для вызова подпрограммы, находящейся в другом сегменте памяти.
		mov dl, 26  ;передача значения 26 в регистр DL
		mov ah, 02h ;передача значение 02h в регистр ah
		int 21h ;прерывание 
		iret ;используется для возврата из прерывания.
	...
	.fname: ; имя
		popa			;Извлечение регистров общего назначения из стека
		call near .sys	;для вызова подпрограммы, находящейся в другом сегменте памяти.
		push ds			;/сохранение в стек
		push cs			; CS - указывает на начало текущего сегмента кода / сохранение
		pop ds			;  сохраним значение сегмента в ds
		mov ax, 0900h
		mov dx, myfname
		int 21h ;прерывание 
		pop ds ; сохраним значение сегмента в ds
	...
	.sname: ; фамилия
		popa ;Извлечение регистров общего назначения из стека
		call near .sys ;для вызова подпрограммы, находящейся в другом сегменте памяти.
		push ds ; сохраннение в стек
		push cs ; сохраннение в стек
		pop ds ; сохраним значение сегмента в ds
		mov ax, 0900h
		mov dx, mysname
		int 21h
	...
	init:              ;------------; установка нового обработчика ----------------------|
		push es ; сохранение
		...
		cmp bx, new09h
		jz already
		pop es  ;сохраним значение сегмента в es
		push ds ; восстанавливаем ВСЕ без исключения регистры
		push cs ;сохранение
		pop ds ; сохраним значение сегмента в ds
		...
		int   27h	; выходим из программы, оставляя программу в памяти


	already: ; если программа уже загружена, будет выведенно сообщение
		mov ah, 9
		mov dx, messA
		int 21h
		mov ah, 0
